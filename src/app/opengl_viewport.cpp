...
namespace {
[[nodiscard]] TrackballController::Mode pickMode(Qt::MouseButtons buttons,
...
GLViewport::GLViewport(QQuickItem* parent) : QQuickFramebufferObject(parent) {
    ...
    m_trackball.syncFromCamera(m_cameraState);
...
void GLViewport::mousePressEvent(QMouseEvent* event) {
    ...
    if(mode != TrackballController::Mode::None) {
        m_trackball.begin(event->position(), mode, m_cameraState);
    }
    event->accept();
}
...
void GLViewport::mouseMoveEvent(QMouseEvent* event) {
    ...
    (void)delta; // kept for readability; actual math uses absolute positions.

    // If modifiers/buttons changed mid-drag, switch mode seamlessly.
    const auto mode = pickMode(m_pressedButtons, m_pressedModifiers);
    if(mode != m_trackball.mode()) {
        m_trackball.end();
        m_trackball.setViewportSize(size());
        if(mode != TrackballController::Mode::None) {
            m_trackball.begin(event->position(), mode, m_cameraState);
    ...
    if(mode != TrackballController::Mode::None) {
        m_trackball.setViewportSize(size());
        m_trackball.update(event->position(), m_cameraState);
        Render::RenderSceneController::instance().setCamera(m_cameraState, false);
    }

    // Always update on mouse move to support hover picking/highlight.
    update();
...
void GLViewport::mouseReleaseEvent(QMouseEvent* event) {
    ...
    if(m_trackball.isActive() && m_pressedButtons == Qt::NoButton) {
        m_trackball.end();
...
void GLViewport::wheelEvent(QWheelEvent* event) {
    ...
    if((m_pressedModifiers & Qt::ControlModifier)) {
        ...
        m_trackball.wheelZoom(steps * 5.0f, m_cameraState);
        Render::RenderSceneController::instance().setCamera(m_cameraState, true);
        update();
    }
    event->accept();
...
void GLViewportRenderer::render() {
    ...
    // Hover picking (render uid+type to offscreen FBO then read 1 pixel)
