...
/**
 * @brief Convert a vector of MeshElementRef to a JSON array of {uid, type} objects.
 */
[[nodiscard]] nlohmann::json refsToJson(const std::vector<MeshElementRef>& refs) {
    nlohmann::json arr = nlohmann::json::array();
    for(const auto& ref : refs) {
        auto type_str = meshElementTypeToString(ref.m_type);
        arr.push_back(nlohmann::json{{"uid", static_cast<uint64_t>(ref.m_uid)},
                                     {"type", type_str.value_or("Unknown")}});
    }
    return arr;
}
...
/**
 * @brief Query a mesh element by ref, including node positions and relation data.
 *
...
queryElement(const MeshDocumentImpl& doc, MeshElementUID uid, MeshElementType type) {
    try {
        ...
        // Relation data
        if(type == MeshElementType::Line) {
            info["relatedElements"] = refsToJson(doc.findElementsByLineRef(ref));
        } else {
