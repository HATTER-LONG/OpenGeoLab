...
bool VertexData::operator==(const VertexData& other) const {
    return std::abs(m_posX - other.m_posX) < EPSILON && std::abs(m_posY - other.m_posY) < EPSILON &&
           std::abs(m_posZ - other.m_posZ) < EPSILON &&
           std::abs(m_normalX - other.m_normalX) < EPSILON &&
           std::abs(m_normalY - other.m_normalY) < EPSILON &&
           std::abs(m_normalZ - other.m_normalZ) < EPSILON;
}

std::size_t VertexDataHash::operator()(const VertexData& vertex) const {
    return std::hash<float>()(vertex.m_posX) ^ (std::hash<float>()(vertex.m_posY) << 1) ^
           (std::hash<float>()(vertex.m_posZ) << 2);
}
...
bool BrepReader::loadBrepFile(const std::string& file_path, TopoDS_Shape& shape) {
...
bool BrepReader::triangulateShape(TopoDS_Shape& shape) {
...
bool BrepReader::extractTriangleData(const TopoDS_Shape& shape,
...
std::shared_ptr<Geometry::GeometryData> BrepReader::read(const std::string& file_path) {
    ...
    try {
        ...
        TopoDS_Shape shape;
        if(!loadBrepFile(file_path, shape)) {
            return nullptr;
        }
