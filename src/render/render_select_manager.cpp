...
void RenderSelectManager::setHoverEntity(uint64_t uid,
    ...
    bool changed = false;
    {
        ...
        if(m_hoveredEntity.m_uid != uid || m_hoveredEntity.m_type != type ||
           m_hoveredPartUid != partUid || m_hoveredWireUid != wireUid) {
            m_hoveredEntity = PickResult{uid, type};
            m_hoveredPartUid = partUid;
            m_hoveredWireUid = wireUid;
            changed = true;
        }
    }
    if(changed) {
...
bool RenderSelectManager::isPartHovered(uint64_t partUid) const {
    if(partUid == 0) {
        return false;
    }
    std::lock_guard lock(m_mutex);
    return m_hoveredPartUid == partUid;
}

bool RenderSelectManager::isWireHovered(uint64_t wireUid) const {
    if(wireUid == 0) {
        return false;
    }
    std::lock_guard lock(m_mutex);
    return m_hoveredWireUid == wireUid;
}
...
bool RenderSelectManager::isPartSelected(uint64_t partUid) const {
    if(partUid == 0) {
        return false;
    }
    std::lock_guard lock(m_mutex);
    return m_currentSelections.contains(PickResult{partUid, RenderEntityType::Part});
}

bool RenderSelectManager::isWireSelected(uint64_t wireUid) const {
    if(wireUid == 0) {
        return false;
    }
    std::lock_guard lock(m_mutex);
    return m_currentSelections.contains(PickResult{wireUid, RenderEntityType::Wire});
}
...
void RenderSelectManager::setHoveredWireEdges(const std::vector<uint64_t>& edgeUids) {
    std::lock_guard lock(m_mutex);
    m_hoveredWireEdgeUids.clear();
    m_hoveredWireEdgeUids.insert(edgeUids.begin(), edgeUids.end());
}

bool RenderSelectManager::isEdgeInHoveredWire(uint64_t edgeUid) const {
    if(edgeUid == 0) {
        return false;
    }
    std::lock_guard lock(m_mutex);
    return m_hoveredWireEdgeUids.contains(edgeUid);
}

void RenderSelectManager::addSelectedWireEdges(uint64_t wireUid,
                                               const std::vector<uint64_t>& edgeUids) {
    std::lock_guard lock(m_mutex);
    m_selectedWireEdges[wireUid].insert(edgeUids.begin(), edgeUids.end());
}

void RenderSelectManager::removeSelectedWireEdges(uint64_t wireUid) {
    std::lock_guard lock(m_mutex);
    m_selectedWireEdges.erase(wireUid);
}
...
bool RenderSelectManager::isEdgeInSelectedWire(uint64_t edgeUid) const {
    if(edgeUid == 0) {
        return false;
    }
    std::lock_guard lock(m_mutex);
    for(const auto& [_, edges] : m_selectedWireEdges) {
        if(edges.contains(edgeUid)) {
            return true;
        }
    }
    return false;
