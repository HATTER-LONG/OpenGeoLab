...
void SceneRenderer::encodeEntityId(Geometry::EntityUID entityUid,
                                   Geometry::EntityType entityType,
                                   uint8_t& r,
                                   uint8_t& g,
                                   uint8_t& b,
                                   uint8_t& a) {
    ...
    r = static_cast<uint8_t>(entityUid & 0xFF);
    g = static_cast<uint8_t>((entityUid >> 8) & 0xFF);
    b = static_cast<uint8_t>((entityUid >> 16) & 0xFF);
    a = static_cast<uint8_t>(entityType);
}

bool SceneRenderer::decodeEntityId(uint8_t r,
                                   uint8_t g,
                                   uint8_t b,
                                   uint8_t a,
                                   Geometry::EntityUID& entityUid,
                                   Geometry::EntityType& entityType) {
    ...
    entityUid = static_cast<Geometry::EntityUID>(r) | (static_cast<Geometry::EntityUID>(g) << 8) |
                (static_cast<Geometry::EntityUID>(b) << 16);
    entityType = static_cast<Geometry::EntityType>(a);
    return true;
}
...
void SceneRenderer::renderIdBuffer(const QMatrix4x4& mvp) {
    ...
    auto renderBuffersWithId = [&](std::vector<MeshBuffers>& buffers, float pointSize) {
        m_idShader->setUniformValue(m_idPointSizeLoc, pointSize);
    ...
    // Render faces
    renderBuffersWithId(m_faceMeshBuffers, 1.0f);

    // Render edges with increased line width for better picking
    glLineWidth(4.0f);
    renderBuffersWithId(m_edgeMeshBuffers, 1.0f);

    // Render vertices with large point size for picking
    renderBuffersWithId(m_vertexMeshBuffers, 10.0f);
