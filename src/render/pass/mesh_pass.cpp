...
void main() {
    ...
    float ambient = 0.4;
    // Half-lambert: wraps diffuse to avoid harsh falloff and concentrated hotspot
    float rawDot = abs(dot(N, V));
    float headlamp = rawDot * 0.5 + 0.5;
    ...
    float lighting = ambient + headlamp * 0.5 + skyLight + groundBounce;
