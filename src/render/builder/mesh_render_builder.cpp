...
void pushLine(RenderPassData& pass, const Vec3f& a, const Vec3f& b, const PrimitiveStyle& style) {
    const Vec3f zero{0.0f, 0.0f, 0.0f};
    pushVertex(pass, a, zero, style);
    pushVertex(pass, b, zero, style);
}

void pushEdge(RenderPassData& pass, const Vec3f& a, const Vec3f& b, const PrimitiveStyle& style) {
...
void appendSurfaceTriangles(BuildContext& ctx) {
    constexpr float DARKEN_FACTOR = 1.0f;
    const RenderColor fallback_color{0.55f, 0.65f, 0.75f, 1.0f};
    ...
    for(const auto& elem : ctx.m_input.m_elements) {
        ...
        RenderColor surface_color = fallback_color;
        if(elem.partUid() != 0) {
        ...
        const PrimitiveStyle style{surface_color, PickId::encode(render_type, elem.elementUID())};
...
void appendWireframeEdges(BuildContext& ctx) {
    const RenderColor wire_color = ctx.m_colorMap.getMeshLineColor();
    ...
    for(const auto& elem : ctx.m_input.m_elements) {
        ...
        const PrimitiveStyle style{wire_color,
                                   PickId::encode(RenderEntityType::MeshLine, elem.elementUID())};
...
void appendNodePoints(BuildContext& ctx) {
    const RenderColor node_color = ctx.m_colorMap.getMeshNodeColor();
    ...
    for(const auto& node : ctx.m_input.m_nodes) {
        ...
        const PrimitiveStyle style{node_color,
                                   PickId::encode(RenderEntityType::MeshNode, node.nodeId())};
