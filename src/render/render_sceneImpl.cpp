...
void RenderSceneImpl::synchronize(const SceneFrameState& state) {
    ...
    if(renderData.m_geometryDirty) {
        m_pickResolver.rebuild(m_geometryPass.triangleRanges(), m_geometryPass.lineRanges(),
                               m_geometryPass.pointRanges(), renderData.m_pickData);
    }
}
...
void RenderSceneImpl::render() {
    ...
    m_geometryPass.render(m_frameState.viewMatrix, m_frameState.projMatrix, m_frameState.cameraPos,
                          m_frameState.xRayMode);
    m_meshPass.render(m_frameState.viewMatrix, m_frameState.projMatrix, m_frameState.cameraPos,
                      m_frameState.xRayMode);
}
...
void RenderSceneImpl::processHover(int pixel_x, int pixel_y) {
    ...
    const GeometryPickInput geomInput{m_geometryPass.gpuBuffer(), m_geometryPass.triangleRanges(),
                                      m_geometryPass.lineRanges(), m_geometryPass.pointRanges()};
    const MeshPickInput meshInput{m_meshPass.gpuBuffer(), m_meshPass.surfaceVertexCount(),
                                  m_meshPass.wireframeVertexCount(), m_meshPass.nodeVertexCount()};
    m_pickPass.renderToFbo(m_frameState.viewMatrix, m_frameState.projMatrix, geomInput, meshInput,
                           effectiveMask);
...
void RenderSceneImpl::processPicking(const PickingInput& input) {
    ...
    // Render to pick FBO using cached matrices
    const GeometryPickInput geomInput2{m_geometryPass.gpuBuffer(), m_geometryPass.triangleRanges(),
                                       m_geometryPass.lineRanges(), m_geometryPass.pointRanges()};
    const MeshPickInput meshInput2{m_meshPass.gpuBuffer(), m_meshPass.surfaceVertexCount(),
                                   m_meshPass.wireframeVertexCount(), m_meshPass.nodeVertexCount()};
    m_pickPass.renderToFbo(m_frameState.viewMatrix, m_frameState.projMatrix, geomInput2, meshInput2,
                           effectiveMask);
