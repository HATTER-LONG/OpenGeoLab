...
/**
 * @brief Convert EntityType to string representation
 * @param type Entity type enum
 * @return Human-readable type name
 */
[[nodiscard]] const char* entityTypeToString(EntityType type) {
    switch(type) {
...
/**
 * @brief Build entity info JSON for a single entity
 * @param entity Entity to describe
 * @param document Document containing the entity
 * @return JSON object with entity details
 */
...
[[nodiscard]] nlohmann::json QueryEntityAction::execute(const nlohmann::json& params,
                                                        Util::ProgressCallback progress_callback) {
    nlohmann::json response;

    if(progress_callback && !progress_callback(0.1, "Querying entity...")) {
    ...
    // Check for batch query (entity_ids array)
