...
public:
    ...
    /// Re-sync internal state from an externally modified camera.
    ...
    /// steps: typically angleDelta.y/120. Positive means wheel-up.
...
private:
    ...
    // steps > 0 means zoom-in; steps < 0 means zoom-out.
...
private:
    ...
    float m_zoomBase{0.90f};          // pow(base, +steps) => zoom in
    float m_zoomPixelsPerStep{60.0f}; // drag pixels mapped to 1 zoom step
