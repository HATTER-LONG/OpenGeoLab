...
public:
     ...
     */
    void
    setHoverEntity(uint64_t uid, RenderEntityType type, uint64_t partUid = 0, uint64_t wireUid = 0);
    ...
    [[nodiscard]] bool isPartHovered(uint64_t partUid) const;
    /** @brief Check whether a wire is currently hovered. */
    [[nodiscard]] bool isWireHovered(uint64_t wireUid) const;

    /**
     * @brief Set the edge UIDs belonging to the currently hovered wire.
     * @param edgeUids All edge UIDs that form the hovered wire's closed loop.
     */
    void setHoveredWireEdges(const std::vector<uint64_t>& edgeUids);

    /** @brief Check whether an edge belongs to the currently hovered wire. */
    [[nodiscard]] bool isEdgeInHoveredWire(uint64_t edgeUid) const;
    ...
    [[nodiscard]] bool isPartSelected(uint64_t partUid) const;
    /** @brief Check whether a wire is in the selection set. */
    [[nodiscard]] bool isWireSelected(uint64_t wireUid) const;

    /**
     * @brief Record all edge UIDs belonging to a selected wire.
     * @param wireUid The wire being selected.
     * @param edgeUids All edge UIDs forming the wire's closed loop.
     */
    void addSelectedWireEdges(uint64_t wireUid, const std::vector<uint64_t>& edgeUids);

    /** @brief Remove edge UIDs associated with a deselected wire. */
    void removeSelectedWireEdges(uint64_t wireUid);
    ...
    [[nodiscard]] bool isEdgeInSelectedWire(uint64_t edgeUid) const;
