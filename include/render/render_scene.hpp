...
struct SceneFrameState {
    const RenderData* renderData{nullptr}; ///< Pointer to controller's render data snapshot
    QVector3D cameraPos;                   ///< Camera world-space position (for lighting)
    QMatrix4x4 viewMatrix;                 ///< View transformation matrix
    QMatrix4x4 projMatrix;                 ///< Projection transformation matrix
    bool xRayMode{false};                  ///< X-ray (semi-transparent surfaces) mode
