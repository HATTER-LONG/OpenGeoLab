...
1. 了解当前实现，完成 Geometry 层功能开发：
   - 完善 geometry document 对外接口，为 io/reader 中的 brep step 提供几何加载功能。
   - 为 geometry document 提供几何编辑功能接口，例如 trim 等，设计一套当几何发生改变后重构 entity 层数据的机制。
   - 为 geometry document 提供获取 render 数据的接口，以供 opengl 渲染使用。
