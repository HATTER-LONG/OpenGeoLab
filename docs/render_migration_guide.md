...
> 基于 commit `83797b5` (FBO GPU Picking)、`c02acda` (Render & Selection Enhancement) 和 `5159e75` (MeshLine 重构为正式 MeshElement) 的完整渲染架构分析与分模块移植方案。
...
/// 网格线关系查询已移至 MeshDocument (通过 findLinesByNodeId / findElementsByLineRef 等方法)。
struct PickResolutionData {
    ...
    void clear();
    void clearGeometry();
    void clearMesh();  // 当前为空 — 网格关系数据由 MeshDocument 管理
};
```
...
2. **线框生成**: 遍历 Line 类型的 MeshElement → 线段顶点
   - 颜色 = `ColorMap::getMeshLineColor()`
   - PickId = encode(MeshLine, lineElementUID) — 直接使用 Line 元素的 UID
   - Line 元素由 `MeshDocument::buildEdgeElements()` 在网格生成后自动创建和去重
   - 节点↔线↔元素关系通过 MeshDocument 的关系查询方法获取
3. **节点生成**: 遍历所有节点 → 点顶点
...
struct GeometryPickInput {
    GpuBuffer& m_buffer;
    const vector<DrawRangeEx>& m_triRanges;
    const vector<DrawRangeEx>& m_lineRanges;
    const vector<DrawRangeEx>& m_pointRanges;
};
...
void renderToFbo(PickFbo& fbo,
                 const QMatrix4x4& vp,
                 RenderEntityTypeMask pickMask,
                 const GeometryPickInput& geo,
                 const MeshPickInput& mesh);
```

内部通过 `renderGeometryPick()` 和 `renderMeshPick()` 辅助函数分别处理:
```
仅渲染 pickMask 匹配的图元:
几何缓冲 (GeometryPickInput):
...
- `PickResolutionData` (嵌套于 `RenderData.m_pickData`) 包含几何域 pick 辅助查找表
  (Edge→Wire, Wire→Edge, Wire→Face); 网格域关系数据由 `MeshDocument` 管理
- `MeshDocument` 提供节点↔线↔元素双向关系查询:
  `findLinesByNodeId()`, `findElementsByNodeId()`,
  `findElementsByLineRef()`, `findLinesByElementRef()`
...
- [ ] `PickResolutionData` 的 clear/clearGeometry/clearMesh 正确清理对应域数据 (clearMesh 为空操作)
...
- [ ] Line 元素使用 `generateMeshElementUID(Line)` 生成的 UID 作为 PickId
- [ ] `MeshDocument::buildEdgeElements()` 正确创建和去重 Line 元素
- [ ] 节点↔线↔元素关系查询方法返回正确结果
...
- [ ] `PickPass::renderToFbo()` 使用 GeometryPickInput/MeshPickInput 参数结构体
