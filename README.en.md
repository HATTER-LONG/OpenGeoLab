...
- Mesh entity picking (Node/Line/Element) with hover and selection highlighting via GPU shader-based pickId comparison; hovered/selected mesh elements display highlight overlay even in wireframe-only mode (highlight-only surface pass discards non-highlighted fragments); MeshLine uses sorted node-pair UIDs for stable shared-edge picking
...
- Mesh wireframe MeshLine picking uses sorted node-pair UIDs (`min(n0,n1)<<32 | max(n0,n1)`), so shared edges across different elements have the same pickId for stable picking.
...
- MeshLine query fix: MeshLine uses composite node-pair UIDs (`min(n0,n1)<<32 | max(n0,n1)`); when no matching Line element is found, the query auto-decodes the composite UID into two endpoint node positions. MeshQueryPage displays the node pair instead of the raw large UID value.
